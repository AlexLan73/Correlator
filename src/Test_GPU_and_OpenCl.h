...
void checkCommandBuffersSupport(cl_device_id device) {
    ...
    if (strstr(extensions, "cl_khr_command_buffer")) {
        ...
        // Дополнительная информация (проверка через расширения уже выполнена выше)
        // Примечание: CL_DEVICE_COMMAND_BUFFER_CAPABLE_KHR может быть недоступно
        // в зависимости от версии OpenCL headers
    } else {
